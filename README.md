...
* **`newOverlayInsertedAt (Master:WAT_Name, OverlayOrNameOrIndex?:WAT_Overlay|WAT_Name|number):WAT_Overlay`**<br>creates a new overlay based on the given `Master` and inserts it at the given `InsertionPoint`. `InsertionPoint` may be set to a concrete overlay, an overlay name or an overlay's index in the list of overlays in this applet. If it is missing, the new overlay is appended to the list of already existing overlays
